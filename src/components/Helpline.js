      ...
      <Box className="helpline-modal" sx={{ ...modalStyle }}>
        ...
        {helplineData.map((item, index) => (
          <Box className="helpline-item" key={index} sx={{ marginBottom: '20px' }}>
            ...
            <Typography variant="h6" sx={{ color: 'blue', marginTop: '10px' }}>{item.title}</Typography>
            {item.phone && <Typography variant="body1">Phone: <MuiLink href={`tel:${item.phone}`} color="primary">{item.phone}</MuiLink></Typography>}
            {item.email && <Typography variant="body1">Email: <MuiLink href={`mailto:${item.email}`} color="primary">{item.email}</MuiLink></Typography>}
            {item.quote && <Typography variant="body2" sx={{ fontStyle: 'italic', marginTop: '10px' }}>"{item.quote}"</Typography>}
          </Box>
        ))}
      </Box>
    </Modal>
